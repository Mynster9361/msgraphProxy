...
$response = Invoke-RestMethod -Method POST -Uri 'https://api.github.com/repos/Mynster9361/msgraphProxy/releases' -Headers @{
	...
	body = "Releasing v$moduleVersion of the msgraphProxy module."
	make_latest = 'true'
...
Invoke-RestMethod -Method POST -Uri "$($response.assets_url -replace 'api\.github\.com', 'uploads.github.com')?name=msgraphProxy.zip" -Headers @{
	Authorization = "Bearer $env:GH_TOKEN"
	Accept = 'application/vnd.github+json'
	'X-GitHub-Api-Version' = '2022-11-28'
	'Content-Type' = 'application/octet-stream'
} -InFile "$WorkingDirectory\publish\msgraphProxy.zip"
