...
if ($LocalRepo) {
	Write-Host "Creating Nuget Package for module: 'msgraphProxy' at '$(Get-Location)"
	Publish-PSFModule -Path "$($publishDir)\msgraphProxy" -DestinationPath . -SkipDependenciesCheck
	return
}

# Publish to Gallery
Write-Host "Publishing the msgraphProxy module to $($Repository)"
$param = @{
	Path                  = "$($publishDir)\msgraphProxy"
